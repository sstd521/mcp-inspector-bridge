            ...
            if (!targetWin.__mcpInspector) {
                targetWin.__mcpInspector = {
                    ...
                    sendNodeSelected: (uuid: string, cancelled = false) => {
                        if (targetWin === window) ipcRenderer.sendToHost('node-picker-selected', uuid, cancelled);
                        else window.postMessage({ __mcp_ipc_proxy: true, channel: 'node-picker-selected', args: [uuid, cancelled] }, '*');
                    },
                    sendClearSelection: () => {
